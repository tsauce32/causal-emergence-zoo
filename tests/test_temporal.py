...
def test_temporal_features_reset_at_declared_time_gap():
    rows = [("a", 0.0, [1.0]), ("a", 1.0, [3.0]), ("a", 10.0, [10.0]), ("a", 11.0, [14.0])]
    transformed = list(derive_temporal_features(rows, feature_names=["x"], differences=[1], max_gap=2.0))
    assert transformed == [("a", 1.0, [3.0, 2.0]), ("a", 11.0, [14.0, 4.0])]
